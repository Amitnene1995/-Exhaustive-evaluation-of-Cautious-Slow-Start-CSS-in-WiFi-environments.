...
OnoeWifiManager::UpdateMode (OnoeWifiRemoteStation *station)
{
  ...
  switch (dir)
    {
    case 0:
      if (enough && station->m_tx_upper > 0)  //when number of retries are more than 10% but not too much aswell
        {
          station->m_tx_upper--;
        }
      break;
  ...
  if (nrate != station->m_txrate) //if nrate changes increase or decrease
    {
  ...
  else if (enough)  //enough pakets are send and no change in nrate
    {
